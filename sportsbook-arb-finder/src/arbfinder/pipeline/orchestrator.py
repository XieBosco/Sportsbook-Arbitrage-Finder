...
async def run(config: AppConfig) -> None:
    ...
    # ------ Launch UI server (if websocket sink is enabled) ------
    ui_server_task: asyncio.Task | None = None
    if components.app is not None:
        ...
        ui_server_task = asyncio.create_task(
            server.serve(), name="ui-server",
        )
        logger.info(
            "UI server started at http://%s:%d",
            config.server.host,
            config.server.port,
        )
    ...
    health_task = asyncio.create_task(
        _health_monitor(components.health_tracker, stop_event, components.scanner),
        name="health-monitor",
    )
    ...
    if ui_server_task is not None:
        all_tasks.append(ui_server_task)
