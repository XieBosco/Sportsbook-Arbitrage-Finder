...
LOG_DIR = "C:\\Users\\fiona\Desktop\\arbitrage_tool\\sportsbook-arb-finder\\scripts\\draftkings_messages"
...
def on_message(ws, message):
    ...
    if data.get("method") == "Network.webSocketFrameReceived":
        ...
        # Filter for live odds WS
        if "sportsbook-ws" in url and "draftkings.com" in url:
            if payloadData:
    ...
    if data.get("method") == "Network.responseReceived":
        ...
        if "api/sportscontent" in url and "draftkings.com" in url and "application/json" in mime:
...
def main():
    ...
    print(f"Waiting for DraftKings traffic...")
    print(f"Make sure to REFRESH the DraftKings Sportsbook page to capture the initial JSONs!")
    ws.run_forever()
