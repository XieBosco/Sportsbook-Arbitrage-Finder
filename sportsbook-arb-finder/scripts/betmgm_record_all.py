...
LOG_DIR = "C:\\Users\\fiona\\Desktop\\arbitrage_tool\\sportsbook-arb-finder\\scripts\\betmgm_messages"
...
def on_message(ws, message):
    ...
    if data.get("method") == "Network.webSocketFrameReceived":
        ...
        if "cds-push" in url and "betmgm.ca" in url:
            if payloadData:
    ...
    if data.get("method") == "Network.responseReceived":
        ...
        if "fixture-view" in url and "betmgm.ca" in url and "application/json" in mime:
...
def main():
    ...
    print(f"Waiting for BetMGM traffic...")
    print(f"Make sure to REFRESH the BetMGM Sportsbook page to capture the initial JSONs!")
    ws.run_forever()
